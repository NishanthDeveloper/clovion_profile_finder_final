...
class TwentyTwoEducation extends StatefulWidget {
  @override
  State<TwentyTwoEducation> createState() => _TwentyTwoEducationState();
}

class _TwentyTwoEducationState extends State<TwentyTwoEducation> {
  ...
  Future _updateDetails() async {
    ...
    for (String item in removeCommaSibName) {
      request.files
          .add(http.MultipartFile.fromString('education_schooledit', item));
    }

    for (String item in removeCommaSibRelation) {
      request.files.add(http.MultipartFile.fromString('education_year', item));
    }
    for (String item in removeCommaSibOccupation) {
      request.files
          .add(http.MultipartFile.fromString('education_course', item));
    }
  ...
  _getDataFromServer() async {
    ...
    if (response.statusCode == 200) {
      setState(() {
        ...
        String out = des["education_school"].replaceAll("[", "");
        ...
        out = des["education_year"].replaceAll("[", "");
        out1 = out.replaceAll("]", "");
        ...
        out = des["education_course"].replaceAll("[", "");
        out1 = out.replaceAll("]", "");
        out2 = out1.replaceAll(' ', "");
        out3 = out2.replaceAll("'", "");
                ...
                Container(
                  ...
                  child: TextFormField(
                    ...
                    decoration: const InputDecoration(
                        contentPadding: EdgeInsets.all(10),
                        border: InputBorder.none,
                        hintText: "Enter the Course Of Education"),
                  ),
                ),
