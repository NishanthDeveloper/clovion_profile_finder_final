...
class FoodTasteTwentySevenScreen extends StatefulWidget {
  @override
  State<FoodTasteTwentySevenScreen> createState() =>
      _FoodTasteTwentySevenScreenState();
}

class _FoodTasteTwentySevenScreenState
    extends State<FoodTasteTwentySevenScreen> {
  static const String _title = "Food Taste";
  ...
  Future _updateDetails() async {
    ...
    for (String item in _myNonInterestSelected) {
      request.files.add(http.MultipartFile.fromString('food_taste', item));
    }
  ...
  _getDataFromServer() async {
    ...
    if (response.statusCode == 200) {
      setState(() {
        ...
        String out = des["food_taste"].replaceAll("[", "");
