...
class ComplexionTwentySixScreen extends StatefulWidget {
  @override
  State<ComplexionTwentySixScreen> createState() =>
      _ComplexionTwentySixScreenState();
}

class _ComplexionTwentySixScreenState extends State<ComplexionTwentySixScreen> {
  static const String _title = "Complexion";
  ...
  Future _updateDetails() async {
    ...
    for (String item in _myNonInterestSelected) {
      request.files.add(http.MultipartFile.fromString('complexion', item));
    }
  ...
  _getDataFromServer() async {
    ...
    if (response.statusCode == 200) {
      setState(() {
        ...
        String out = des["complexion"].replaceAll("[", "");
